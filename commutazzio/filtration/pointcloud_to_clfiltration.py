...
def points_to_clfiltration_constant_removal(pts:np.array,vertical_removal_input:list,radii:list,max_simplex_dim:int,method:str='cech'):
    """
    Prompts for chatgpt:
    when vertical_removal_input is a constant list of vertices,
    SimplexTree upper and lower can be generated separately,
    where upper is the same as in the function below,
    and lower is generated by removing the points in vertical_removal_input from pts first,
    then generate a simplex tree using the instance's method.
    One thing to take care about is to match the indices of simplcies in upper and lower.
    for cech and rips, the i-th simplex corresponds to the i-th point in the point cloud used to generate the simplex complex.
