...
def pullback(sc_9,sc_7,sc_3):
    """
    Keypad position used. 
    This function generate the pulback of a cospan of simplicial complices.
    """
    for sc in [sc_9,sc_7,sc_3]:
        if type(sc) is list:
            sc = SimplicialComplex(sc)
    if not sc_9.issuperset(sc_7) or not sc_9.issuperset(sc_3):
        raise ValueError
    diff_7_9=[s for s in sc_9.value() if s not in sc_7.value()]
    diff_3_9=[s for s in sc_9.value() if s not in sc_3.value()]
    return sc_9.delete_simplices(diff_7_9+diff_3_9)
    
def lower_left_completion(sc_9,sc_7,sc_3):
    """
    Based on the pullback computed, this function randomly delete some points from the pullback.
    """
    th_low=5
    th_high=10
    pullback_sc=pullback(sc_9,sc_7,sc_3)
    return pullback_sc.random_delete_vertices(num=np.random.randint(th_low,th_high))
...
class SimplicialComplex(object):
    def __init__(self,sc=[]):
        if sc != []: # if the input is not empty, verify whether the form complies with the standard
            if type(sc[0]) is not tuple:
                raise ValueError('Each simplex should be represented by a tuple.')
        self.sc=sc # sc stands for simplicial complex
    ...
    def info_node(self):
        return {'sc':self,'sc_size':self.size()}
        #return {'sc':self.dionysus_form(),'sc_size':self.size()}
    ...
    def __repr__(self):
        description = f"simplicial complex with {self.size()} simplices"
        return description
